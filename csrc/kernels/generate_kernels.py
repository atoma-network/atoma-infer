...
template void run_mha_fwd_splitkv_dispatch<{DTYPE}, {HEAD_DIM}, {IS_CAUSAL}>(Flash_fwd_params &params, cudaStream_t stream, bool show);
"""

KERNEL_IMPL_TEMPLATE_FWD_EMPTY = """#include "flash_fwd_launch_template.h"
...
KERNEL_IMPL_TEMPLATE_FWD_SPLIT_EMPTY = """#include "flash_fwd_launch_template.h"

template<> 
void run_mha_fwd_splitkv_dispatch<{DTYPE}, {HEAD_DIM}, {IS_CAUSAL}>(Flash_fwd_params &params, cudaStream_t stream, bool show) {{
    printf("%s unimplemented!\\n", __FILE__);
}}
"""
...
class Kernel:
    ...
    def template(self) -> str:
        if self.direction == "fwd":
            if self.dtype != "bf16" or self.head_dim != 64:
                return KERNEL_IMPL_TEMPLATE_FWD_EMPTY.format(DTYPE=DTYPE_MAP[self.dtype], HEAD_DIM=self.head_dim, IS_CAUSAL=self.is_causal)
            else:
                return KERNEL_IMPL_TEMPLATE_FWD.format(DTYPE=DTYPE_MAP[self.dtype], HEAD_DIM=self.head_dim, IS_CAUSAL=self.is_causal)
        else:
            if self.dtype != "bf16" or self.head_dim != 64:
                return KERNEL_IMPL_TEMPLATE_FWD_SPLIT_EMPTY.format(DTYPE=DTYPE_MAP[self.dtype], HEAD_DIM=self.head_dim, IS_CAUSAL=self.is_causal)
            else:
                return KERNEL_IMPL_TEMPLATE_FWD_SPLIT.format(DTYPE=DTYPE_MAP[self.dtype], HEAD_DIM=self.head_dim, IS_CAUSAL=self.is_causal)
