...
# and 512 for the whole ladder; LADDER_GATE_STEPS raises the identity steps above 32. The
# flash-attention kernels are a long nvcc build the first time; set FLASH_ATTN_BUILD_DIR to keep
# the build across checkouts.
#
...
[[ -n ${HF_TOKEN:-} ]] || echo "ladder-gates: HF_TOKEN is unset; a gated checkpoint will fail to fetch" >&2
...
echo "==> maximum batch: ${LADDER_GATE_MAX_BATCH:-128}; identity steps: ${LADDER_GATE_STEPS:-32}"
echo "==> commit: $(git rev-parse HEAD)"
echo "==> gpu: $(nvidia-smi --query-gpu=name,driver_version --format=csv,noheader | head -n 1)"

# test-support is what lets the layout gate state a role table that is not the model's.
cargo test -p atoma-engine --locked --features cuda,test-support --test ladder_gates -- --ignored --nocapture
