...
impl CacheConfig { 
    ...
    pub fn from_file_path<P: AsRef<Path>>(path: P) -> Result<Self, CacheConfigError> { 
        ...
        if let Some(num_gpu_blocks_override) = this.num_gpu_blocks_override {
            ...
            let num_gpu_blocks = utils::calculate_num_gpu_blocks(this.block_size, this.gpu_memory_utilization)?;
            this.num_gpu_blocks = Some(num_gpu_blocks);
        }

        this.num_cpu_blocks = Some(this.swap_space_bytes / this.block_size);
    ...
    fn verify_args(&self) -> Result<(), CacheConfigError> {
        ...
        if self.swap_space_fraction.is_none() && self.swap_space_bytes.is_none() {
            return Err(CacheConfigError::InvalidSwapSpace(
                "Cannot leave unspecified both `swap_space_fraction` and `swap_space_bytes`".to_string(),
            ));
        }
...
impl SchedulerConfig {
    ...
    pub fn from_file_path<P: AsRef<Path>>(path: P) -> Result<Self, SchedulerConfigError> {
        let builder = Config::builder().add_source(config::File::with_name(
            config_file_path.as_ref().to_str().unwrap(),
        ));
        let config = builder
...
pub(crate) mod utils { 
    ...
    pub(crate) fn calculate_swap_space(fraction: f32) -> Result<usize, CacheConfigError> {
        ...
        let total_memory = sys_info::mem_info()
            ...
            .total as usize * KB; // Convert from KB to bytes
        ...
        unsafe {
            ...
            for device in 0..device_count {
                ...
                if result != cudaError::cudaSuccess {
                    return Err(CacheConfigError::GpuMemoryQueryError(format!("Failed to set device {}: {:?}", device, result)));
                }
                ...
                if result != cudaError::cudaSuccess {
                    return Err(CacheConfigError::GpuMemoryQueryError(format!("Failed to get memory info for device {}: {:?}", device, result)));
                }
            ...
            let free_memory = per_device_memory.iter()
                .map(|(free, _)| free)
                .min()
                .unwrap();
            let num_gpu_blocks = (*free_memory as f32 * gpu_memory_utilization).floor() as usize / block_size;
